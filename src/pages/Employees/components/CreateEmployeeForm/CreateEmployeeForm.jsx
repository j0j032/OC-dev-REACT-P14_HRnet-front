	...
	return (
		<aside className='create-employee__container'>
			...
			<div className='form-heading'>
				<img onClick={openModal} className='profile-picture picture-m'
					 src={file.preview ? file.preview : imgPlaceholder}
					 alt='preview employee picture'/>
				<h2>Create new employee</h2>
			</div>
			<form onSubmit={handleSubmit(submit)} className='create-employee__form'>
				{isOpenModal && <UploadPicture file={file} setFile={setFile} isOpen={isOpenModal}
											   close={closeModal}/>}
					...
					<editor-fold desc='JOB'>
						<div className='form-section'>
							...
							<SelectInput inputName={'department'}
										 {...register('department')}
										 map={teams.map(item => <option key={item}
																		value={item}>{capitalize(item)}</option>)}
							/>
				...
				<button className='form-btn align-right' disabled={isSubmitting | errors}
						type='submit'>Create
				</button>
			</form>
		</aside>
